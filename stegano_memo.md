...
- まあ、パレットの対応表を抜き出して、各ピクセルが持っているindexと照合してみればLSBも抜き出せるってわけ、めんどい（フレームを固定した場合に限る）
- indexのLSBにも情報は隠せるらしいのだが、frameごとにindexは変化するので、それはまた勉強する機会があればやる
- 逆にパレットは時間経過で情報が変化しない（グローバルパレットの場合）のでLSBに情報を埋め込みやすい特徴

今日のやつをまとめるとpngやbmpがRGB+Aという色情報の方法で、1つのピクセルにつきそれぞれ8ビットのRGBの情報を組み合わせることで、色を表現できるため、このRGBのLSBをいじっても特段画像には変化はないというのが一つ、一方でgifについてはRGB+Aという色情報の方法ではなく、パレットといわれる色の情報のパレットRGBがそれぞれ8ビットずつパレットとして格納されていて、ピクセルの内部にはRGBの情報が入っておらず、indexがはいっている。indexはパレットと対応しており、パレットに応じた色情報でピクセルが表現される。gifはアニメーションなので同じピクセルでも時間経過でindexが変化することがあるのと、パレット（グローバルパレット）に関しては時間経過ではパレット内部の情報は変化しない。
